...
import { Shield, Smartphone, AlertTriangle, Activity, Users, Database } from 'lucide-react';
import { API_BASE_URL, WS_BASE_URL } from '../config';

export default function Dashboard() {
    // Initial "Demo Data" - Simulating 5 previous users
    // Initial empty state, will fill from WebSocket
    const [alerts, setAlerts] = useState([]);
    const [globalRisk, setGlobalRisk] = useState(12);
    const [activeNodes, setActiveNodes] = useState(0);
    // Simulating 5 protected users in the system history
    const [stats, setStats] = useState({ scams: 12, protected: 5, blocked: 9 });
    ...
    useEffect(() => {
        ...
        ws.onmessage = (event) => {
            ...
            if (data.type === 'NEW_THREAT' || data.type === 'AUDIO_CHUNK') {
                ...
                // Increase risk dynamically based on activity
                setGlobalRisk(prev => Math.min(85, prev + 15));
        ...
        <div className="flex-1 p-6 overflow-hidden flex flex-col gap-6 relative h-full">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 z-10">
                {[
                    ...
                    { label: 'Calculated Risk', value: `${globalRisk}%`, icon: Database, color: 'text-purple-400' },
                ].map((stat, idx) => (
